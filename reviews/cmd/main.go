...
import (
	"alexdenkk/books/reviews/app"
	"alexdenkk/books/reviews/model"
	"alexdenkk/books/reviews/pkg/db"
	"flag"
	"log"
)
...
func main() {
	...
	// create app
	reviewsApp := app.New(appDB, []byte(jwt), addr)
	log.Println("app initialized")

	// run
	log.Fatal(reviewsApp.Run())
}

func parseFlags() {
	flag.StringVar(&addr, "address", ":8003", "address and(or) port for app")
